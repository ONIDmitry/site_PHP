...
<html>
<head>
<title>Отдел снабжения института-новость</title>
    ...
    <div class="content">
           ...
           <?php
            ...
            if (!isset($_GET['iden'])){
             ...
             $n=5;
           if ($iden>$n) {
            echo "Такой новости еще не существует))";
           }
           $result=mysql_query("SELECT * FROM news WHERE iden='$iden'");
           ...
           do {
            ...
            <b class="tittle"><h2>'.$data['tittle'].'</h2></b> 
            <p>'.$data['opis'].'</p>
           ...
           while ($data=mysql_fetch_array($result));
            ...
            ?>
            <form method="post" action="mnovost.php">
            <input type="submit" name="backnov" value="Вернуться в Новости"/>
            <form method="post" action="mnovost.php">
            <input type="submit" name="backarh" value="Вернуться в Архив"/>
            </div>  
            <?php
            if(isset($_POST['backnov'])) {
                header("Location:novosti.php");
            }
            if(isset($_POST['backarh'])) {
