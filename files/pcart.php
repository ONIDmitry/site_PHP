...
<div class="right_up">
  <img src="logotip.png"/>
    ...
    <div class="content">
        ...
        if(isset($_POST['bsub'])) {
            $blog=$_POST['log'];
            $mysqlblog=("UPDATE zayavka SET buhcon='yes' WHERE login='$log'");
       ...
       if (isset($_POST['delcort'])) {
        $delcort=mysql_query("DELETE FROM cort WHERE login='$log'");
       }
       
       
      if(isset($_POST['changecort'])) {
        $changecort=mysql_query("UPDATE cort SET confirmed='no' WHERE login='$log'");
      ...
      $prov=mysql_query("SELECT * FROM zayavka WHERE login='$log'");
      if ($mprov=mysql_fetch_array($prov)) {}
     else {   echo'   <p><input type="submit" name="delcort" value="Удалить корзину"/></p>
           <p><input type="submit" name="changecort" value="Изменить корзину"/></p>
          </form> ';}
           ...
           <form method="post" action="pcart.php">
